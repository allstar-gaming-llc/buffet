...
- `deployment.strategy` (optional, verbatim Kubernetes shape) sets the
  Deployment rollout strategy, e.g. `type: Recreate` for single-replica
  services that must never run two pods at once.

- `deployment.volumes` and per-container `volumeMounts` (both optional,
  verbatim) let a service mount volumes, e.g. a PersistentVolumeClaim the
  parent chart ships. Omitted, the rendered output is unchanged.
